...
from ....pb.tensorflow.core.protobuf import meta_graph_pb2
# from tensorflow.core.protobuf import saver_pb2
from ....pb.tensorflow.core.protobuf import saver_pb2
# from tensorflow.core.protobuf import trackable_object_graph_pb2
from ....pb.tensorflow.core.protobuf import trackable_object_graph_pb2
...
class BaseSaverBuilder(object):
  ...
  def _build_internal(self,
    ...
    logging.warning("BaseSaverBuilder._build_internal() TODO: Implement saveables")
    if False:
...
#
# TKTK: Convenience functions for saving/loading a dict of numpy arrays as a TF checkpoint
#
...
def load(ckpt, var_list: Mapping = None):
  ...
  out = {}
  for name, spec in var_list.items():
    tensor = reader.get_tensor(name)
    if spec is not None:
      assert hasattr(spec, 'shape')
      if np.shape(tensor) != np.shape(spec):
        try:
          np.broadcast_shapes(np.shape(tensor), np.shape(spec))
        except ValueError:
          raise ValueError(f"checkpoint shape mismatch for {name!r}: expected {np.shape(spec)}, got {np.shape(tensor)}")
        tensor = np.reshape(tensor, np.shape(spec))
    out[name] = tensor
  return out
